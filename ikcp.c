...
const IUINT32 IKCP_SN_OFFSET = 12;
...
static void ikcp_segment_encode( char *ptr, IKCPSEG *seg )
{
	ikcp_encode32u( ptr, seg->conv );
	ikcp_encode8u( ptr, seg->cmd );
	ikcp_encode8u( ptr, seg->frg );
	ikcp_encode16u( ptr, seg->wnd );
	ikcp_encode32u( ptr, seg->ts );
	ikcp_encode32u( ptr, seg->sn );
	ikcp_encode32u( ptr, seg->una );
	ikcp_encode32u( ptr, UINT32( _countof( seg->data ) ) );
}
