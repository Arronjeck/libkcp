...
UDPSession *UDPSession::DialWithOptions( const char *ip, uint16_t port, size_t dataShards, size_t parityShards ) {
...
UDPSession *UDPSession::dialIPv6( const char *ip, uint16_t port ) {
...
UDPSession *UDPSession::createSession( int sockfd ) {
	bool state = true;
#ifdef _WIN32
...
void UDPSession::Update( uint32_t current ) noexcept {
	for ( ;; ) {
		;
...
void UDPSession::Destroy( UDPSession *sess ) {
	if ( nullptr == sess ) {
		return;
	}
	if ( 0 != sess->m_sockfd ) {
#ifdef _WIN32
		closesocket( sess->m_sockfd );
#else
		close( sess->m_sockfd );
#endif //_WIN32
	}
...
ssize_t UDPSession::Read( char *buf, size_t sz ) noexcept {
	if ( m_streambufsiz > 0 ) {
...
ssize_t UDPSession::Write( const char *buf, size_t sz ) noexcept {
...
int UDPSession::SetDSCP( int iptos ) noexcept {
	iptos = ( iptos << 2 ) & 0xFF;
	return setsockopt( this->m_sockfd, IPPROTO_IP, IP_TOS, reinterpret_cast<const char *>( &iptos ), sizeof( iptos ) );
}

void UDPSession::SetStreamMode( bool enable ) noexcept {
	if ( enable ) {
		this->m_kcp->stream = 1;
	} else {
		this->m_kcp->stream = 0;
...
int UDPSession::out_wrapper( const char *buf, int len, struct IKCPCB *, void *user ) {
	assert( user != nullptr );
...
ssize_t UDPSession::output( const void *buffer, size_t length ) {
	int n = send( m_sockfd, ( const char * )buffer, length, 0 );
	return n;
}
